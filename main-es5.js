...
var routes = [
    ...
    { path: 'detail/:name', loadChildren: function () { return Promise.resolve(/*! import() */).then(__webpack_require__.bind(null, /*! ./layout/detail/detail.module */ "./src/app/layout/detail/detail.module.ts")).then(function (m) { return m.DetailModule; }); } }
];
...
var DetailComponent = /** @class */ (function () {
    function DetailComponent(gitService, route, dialogref, data) {
        var _this = this;
...
var LayoutComponent = /** @class */ (function () {
    function LayoutComponent(breakpointObserver) {
        ...
        breakpointObserver
            ...
            for (var _i = 0, _a = Object.keys(result.breakpoints); _i < _a.length; _i++) {
                var query = _a[_i];
                if (result.breakpoints[query]) {
                    _this.currentScreenSize = _this.displayNameMap.get(query) ? _this.displayNameMap.get(query) : 'Unknown';
                }
            }
        });
    }
...
var SearchComponent = /** @class */ (function () {
    function SearchComponent(gitService, _snackBar, dialog, router) {
        var _this = this;
        this.gitService = gitService;
        ...
        this.reponame = '';
        this.gitService.repoSelectedObs.subscribe(function (res) {
            _this.reponame = res;
        });
    }
    ...
    SearchComponent.prototype.openSnackBar = function (text) {
        this._snackBar.open(text, 'Okay', {
            horizontalPosition: this.horizontalPosition,
            verticalPosition: this.verticalPosition,
        });
    };
    SearchComponent.prototype.showDetail = function (name) {
        var _this = this;
        this.loading = true;
        // this.router.navigate(['/detail', name]);
        this.reponame = name;
        this.gitService.repoSelected.next(this.reponame);
        var dialogRef = this.dialog.open(_detail_detail_component__WEBPACK_IMPORTED_MODULE_6__["DetailComponent"], { data: { name: name } });
        dialogRef.afterClosed().subscribe(function (result) {
            _this.loading = false;
            console.log("Dialog result: " + result);
        });
    };
    SearchComponent.prototype.onSortChange = function () {
    ...
    SearchComponent.prototype.loadData = function (params) {
        var _this = this;
        this.loading = true;
        this.gitService.searchRepo(params)
            ...
            _this.loading = false;
            console.log(res);
            _this.dataRepo = res.items;
            _this.length = res.total_count;
        }, function (err) {
            _this.loading = false;
            console.log(err);
        });
    };
    SearchComponent.prototype.setPageSizeOptions = function (setPageSizeOptionsInput) {
        console.log(setPageSizeOptionsInput);
        if (setPageSizeOptionsInput) {
            this.pageSizeOptions = setPageSizeOptionsInput.split(',').map(function (str) { return +str; });
        }
    };
    SearchComponent.prototype.getServerData = function (event) {
        if (this.form.text == "") {
            this.openSnackBar("Please enter name to search");
        }
        else {
...
var GitService = /** @class */ (function () {
    function GitService(_httpClient) {
    ...
    GitService.prototype.getAll = function () {
        return this._httpClient.get('https://api.github.com/repositories');
    };
    GitService.prototype.searchRepo = function (request) {
    ...
    GitService.prototype.getDetailRepo = function (name) {
        return this._httpClient.get('https://api.github.com/repos/' + name);
    };
    GitService.prototype.getPerUser = function (user) {
